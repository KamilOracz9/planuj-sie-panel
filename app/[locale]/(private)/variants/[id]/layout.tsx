...
const Layout = async ({ children, params }: LayoutProps) => {
    ...
    const existingChannelsPromise = fetchChannelVisibilitiesByModel({ locale, modelId: Number(id), modelType: 'variant' });
    const existingPricesPromise = fetchPricesByModel({ locale, modelId: Number(id), modelType: 'variant' });

    return (
        <VariantContext value={{ variantPromise, productsSelectPromise, existingAttributesPromise, existingChannelsPromise, existingPricesPromise }}>
            {children}
        </VariantContext>
    )
}
